...
let userId: number = 0, post_body: string = "", 
...
const loadProc = async function () {
    try {
        ...
        console.log("#loadProc.userId=", userId);
        const target = await ChatPost.get(Number(post_id));
        let item = target.data
        console.log("#ChatPost.get=", post_id);
        console.log(item);
        post_body = item.body;
...
{
  try {
    const target = await Thread.getItems(post_id);
    console.log("#getThread"); 
console.log(target);    
    threadItems = target.data;
  } catch (e) {
    console.error(e);
